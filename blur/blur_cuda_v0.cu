...
__global__ void Convolve1DHorizontalKernel(const unsigned char* d_src, unsigned char* d_dst,
        int ncols, int nrows, double* d_kernel, size_t kernel_size,
        int kernel_radius)
{
...
void LaunchGaussianSmoothing(const unsigned char* h_src, unsigned char* h_dst,
        int ncols, int nrows, double* h_kernel, size_t kernel_size)
{
    ...
    CUDA_CHECK(cudaMalloc(&d_kernel, kernel_size * sizeof(double)));
    CUDA_CHECK(
            cudaMemcpy(d_kernel, h_kernel, kernel_size * sizeof(double), cudaMemcpyHostToDevice)
            );
